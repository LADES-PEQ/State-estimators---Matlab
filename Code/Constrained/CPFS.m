function [Pxx_filt, x_filt, Particle_filt, Particle_Wf, Particle_priori] = CPFS(F,...
    H, Particle_vector, X_vector, U_vector, Pxx_vector, z_vector, Q_vector, R_vector, Particle_W,...
    Particle_priori, Constraints, Jacob_y, varargin)
% Calculation of the particle filter and smoother (PF&S) based on the
% forward-backward filtering approach proposed by Salau et al. (2012), in
% which the filtering and smoothing estimations follow the algorithms proposed
% by Arulampalam et al. (2002) and Doucet and Johansen (2009), respectively.
%
% Inputs:
...
%
% Note1: The numbering of equations refers to Doucet and Johansen (2009).
%
...
for i = 1:N
    [Pxx_filt(1:nx,1:nx,i+1), x_filt(1:nx,i+1), Particle_filt(1:nx,1:n_particle,i+1),...
        ...
        CPF(F, H, Particle_filt(1:nx,1:n_particle,i), U_vector(1:nu,i),...
        z_vector(1:ny,i+1), Q_vector(1:nx,1:nx,i), R_vector(1:ny,1:ny,i+1),...
        Particle_Wf(1:n_particle,i), Constraints, Pxx_filt(1:nx,1:nx,i), Jacob_y);
end
