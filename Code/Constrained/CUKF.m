function [Pxx_poster,x_poster, Sigma_points, Sigma_priori] = CUKF(F, H, x_prior, ...
    u, Pxx_prior, z, Q, R, Constraints, varargin)
% Calculation of the constrained unscented Kalman filter (CUKF) based on the 
% methodology proposed by Kandepu et al. (2008)
%
% Inputs:
...
%
% Note1: The algorithm is equivalent to the unscented Kalman filter, proposed
% by Wan and van der Merwe (2000), if there are no active constraints in the
% projection function.
%
% Note2: The variables with the prefix "Sigma" have calculations for each 
% sigma point propagated based on the column position.
%
% Note3: Numerical calculations of this implementation and the one proposed
% by Kandepu et al. (2008) lead to the same results; however, some equations
% might differ due to a different state definition.
%
...
% Definition of the number of states, measurements and augmented states
nx = numel(x_prior);
ny = numel(z);
...
Sigma_points = [x_prior x_prior+Gamma*chol(Pxx_prior,'lower')...
                x_prior-Gamma*chol(Pxx_prior,'lower')]; % Eq. 15

for i = 1:2*nx+1
    Sigma_points(1:nx,i) = Projection(Sigma_points(1:nx,i), u, ...
        Constraints, Pxx_prior);
end

% Propagation of sigma points based on the dynamic model
for i = 1:2*nx+1
    Sigma_priori(1:nx,i) = F(Sigma_points(1:nx,i),u); % Eq. 10
    x_priori = x_priori+Wm(i)*Sigma_priori(1:nx,i); % Eq. 11
end

% Calculation of the a priori state error covariance matrix
Pxx_priori = Q; % Eq. 12 part 1
for i = 1:2*nx+1 
    Pxx_priori = Pxx_priori+Wc(i)*(Sigma_priori(1:nx,i)-x_priori)*...
        (Sigma_priori(1:nx,i)-x_priori)'; % Eq. 12 part 2
end

Sigma_update = [x_priori x_priori+Gamma*chol(Pxx_priori,'lower')...
                x_priori-Gamma*chol(Pxx_priori,'lower')]; % Eq. 15

% Calculation of measurements for each sigma point
for i = 1:2*nx+1
    Sigma_output(1:ny,i) = H(Sigma_update(1:nx,i)); % Eq. 13
    y_priori = y_priori+Wm(i)*Sigma_output(1:ny,i); 
end
...
% Note: Calculations implemented between comments about Eqs. 13 and 14 were
% not numbered by Kandepu et al. (2008).

% Update of state error covariance matrix based on information a posteriori
Pxx_poster = Pxx_priori-K*Pyy*K'; % Eq. 14 part 2
Pxx_poster = (Pxx_poster+Pxx_poster')/2;

% Update of state estimation based on information a posteriori
innov = z-y_priori;
x_poster = x_priori+K*innov; % Eq. 14 part 1
