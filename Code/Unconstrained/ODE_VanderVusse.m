function dxdt = ODE_VanderVusse(t,x,u)
% Definition of the system of ordinary differential equations corresponding
% to Van der Vusse reactions in a jacketed continuous stirred tank reactor
% (CSTR), according to modeling from Katt and Engell (1993).
%
% Reactions:
...
dxdt(1,:) = (Ca0-x(1,:))*F/Vr-r1-r3;
dxdt(2,:) = (-x(2,:))*F/Vr+r1-r2;
dxdt(3,:) = (T0-x(3,:))*F/Vr+kw*Ar*(x(4,:)-x(3,:))./(rho*cp*Vr)-...
            (dH1*r1+dH2*r2+dH3*r3)./(rho*cp);
dxdt(4,:) = (Q+kw*Ar*(x(3,:)-x(4,:)))/(mK*cpk);
end
