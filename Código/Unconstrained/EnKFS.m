function [Pxx_filt, x_filt, Ensemble_filt, Ensemble_priori] = EnKFS(F, H,...
    Ensemble_vector, U_vector, Pxx_vector, z_vector, Q_vector, R_vector,...
    Jacob_y, X_vector, Ensemble_priori, varargin)
% Calculation of the ensemble Kalman filter and smoother (EnKF&S) for a 
% forward-backward filtering approach, in which the filtering and smoothing
% estimations follow the algorithms proposed by Evensen (2003) and Raanes 
% (2016), respectively.
%
% Inputs:
...
for i = 1:N
    [Pxx_filt(1:nx,1:nx,i+1), x_filt(1:nx,i+1), Ensemble_filt(1:nx,1:n_ensemble,i+1),...
        Ensemble_priori(1:nx,1:n_ensemble,i)] = EnKF(F, H, Ensemble_filt(1:nx,1:n_ensemble,i),...
        U_vector(1:nu,i), z_vector(1:ny,i+1),...
        Q_vector(1:nx,1:nx,i), R_vector(1:ny,1:ny,i+1), Jacob_y);
end
