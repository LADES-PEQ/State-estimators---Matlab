...
n_scen = 8*5; % Number of different scenarios
...
for flag_ode = 1:3
    ...
    for k = 1:N_k-1
        ...
        Plant{flag_ode}.Par(1:npar,k+1) = Par + normrnd(0,w_U);

        % System update
        if flag_ode >1
            Plant{flag_ode}.Par(3,k+1) = 5.7;
        end
    ...
    for i = 1:n_sim
        [Results{flag_ode}.X_sim(1:nest, 1:N_k,i), Results{flag_ode}.Y_sim(1:ny, 1:N_k,i),...
            ...
            ~] = Estimation_Unconst(F,...
            H, [X0;Par(pos_var)], Plant{flag_ode}.Par(pos_u,:), Plant{flag_ode}.Z,...
...
save(strcat(dir_save,"Unconst.mat"),'Plant','Results', 'n_sim_scen', 'N_smo', 'N_points');